...
func EnsureAdminPassword() {
	if envPw := strings.TrimSpace(os.Getenv("ADMIN_PASSWORD")); envPw != "" {
		if strings.TrimSpace(config.Load().AdminPassword) != envPw {
			if err := config.WriteSettings(map[string]any{"admin_password": envPw}); err != nil {
				log.Printf("[Admin] 写入环境变量指定的管理员密码失败：%v", err)
				return
			}
		}
