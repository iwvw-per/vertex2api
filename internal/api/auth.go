...
func (m *APIKeyManager) LoadKeys() bool {
	...
	if f, err := os.Open(m.keysFile); err == nil {
	...
	// 从环境变量 VPROXY_API_KEYS_ENV 注入额外密钥（分号分隔，格式 name:key 或 name:key:desc）。
	// 用于无持久化存储的环境（如 Koyeb），文件不存在或为空时也能注入。
	if envKeys := strings.TrimSpace(os.Getenv("VPROXY_API_KEYS_ENV")); envKeys != "" {
		for _, part := range strings.Split(envKeys, ";") {
			...
			key, name := strings.TrimSpace(seg[0]), "env"
			if len(seg) >= 2 && strings.TrimSpace(seg[0]) != "" && strings.TrimSpace(seg[1]) != "" {
				name, key = strings.TrimSpace(seg[0]), strings.TrimSpace(seg[1])
			}
			if !strings.HasPrefix(key, "sk-") {
				continue
			}
